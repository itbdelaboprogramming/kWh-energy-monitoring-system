...
class node:
    ...
    def reset_read_attr(self):
        ...
        self.Frequency_ref              = 0
        self.Output_Frequency           = 0
        self.Output_Current             = 0
        self.new_Output_Current         = 0
        self.Output_Voltage_ref         = 0
        self.DC_Bus_Voltage             = 0
        self.Output_Power_kW            = 0
        self.new_Output_Power_kW        = 0
        self.Output_Voltage             = 0
        self.Input_Current              = 0
        self.new_Input_Current          = 0
    ...
    def save_read(self,response,save,num):
        register = self.handle_sign(response.registers)
        # Save responses to object's attributes
        if save == 1:
            self.Frequency_ref      = register[0]/100     # Hz
            self.Output_Frequency   = register[1]/100     # Hz
            self.Output_Current     = register[2]/100     # Amps
            self.new_Output_Current = round(0.7*register[2]/100,2)     # Amps
            self.Output_Voltage_ref = register[5]/10      # Volts
            self.DC_Bus_Voltage     = register[6]         # Volts
            #self.Output_Power_kW    = register[7]  # kW
        if save == 2:
            self.Output_Voltage     = register[0]/10      # Volts
            self.Output_Power_kW    = round(self.Output_Current*self.Output_Voltage*(3**(0.5))/1000,2)
            self.new_Output_Power_kW    = round(0.91*self.new_Output_Current*self.Output_Voltage*(3**(0.5))/1000,2)
            if self.DC_Bus_Voltage != 0:
                self.Input_Current = round(0.65*self.Output_Power_kW*1000/self.DC_Bus_Voltage,2)
                self.new_Input_Current = round(1.835*self.new_Output_Power_kW*1000/self.DC_Bus_Voltage-9,2)
                if self.new_Input_Current < 0.25:
                    self.new_Input_Current = 0
            else:
                self.Input_Current = 0
                self.new_Input_Current = 0
