...
class node:
    ...
    def save_read(self,response,save,num):
        ...
        if save == 1:
            self.Frequency_ref      = response.registers[0]/100     # Hz
            self.Output_Frequency   = response.registers[1]/100     # Hz
            self.Output_Current     = response.registers[2]/100     # Amps
            self.Output_Voltage_ref = response.registers[5]/10      # Volts
            self.DC_Bus_Voltage     = response.registers[6]         # Volts
            #self.Output_Power_kW    = response.registers[7]  # kW
        if save == 2:
            self.Output_Voltage     = response.registers[0]/10      # Volts
