...
class node:
    ...
    def save_read_1(self,response):
        self.Current            = response.registers[1]/1000    # Amps
        self.Voltage            = response.registers[3]/100     # Volt
        self.ActivePowerW       = response.registers[5]         # Watt
        self.ReactivePowerVAr   = response.registers[7]         # VAr
        self.PowerFactor        = response.registers[11]/10000
        self.Frequency          = response.registers[13]/100    # Hz
    ...
    def save_read_2(self,response):
        ...
        self.TotalReactiveEnergyKVArh   = response.registers[27]/10    # kVArh

    def enable_register_access(self,client):
        # Send the command with function_code = 0x06 (6)
        response = client.write_register(address=4945, value=0x0001, unit=self.unit)
        time.sleep(3)
        return response
    
    def reset_all_values(self,client):
        # Send the command with function_code = 0x06 (6)
        response = client.write_register(address=5330, value=0x0001, unit=self.unit)
        time.sleep(3)
        return response
    ...
    def send_command(self,client,command,param=None):
        ...
        if command == "read_measurement":
            response = client.read_holding_registers(address=1, count=14, unit=self.unit)
            self.save_read_1(response)
            time.sleep(self.client_transmission_delay)
            response = client.read_holding_registers(address=129, count=28, unit=self.unit)
